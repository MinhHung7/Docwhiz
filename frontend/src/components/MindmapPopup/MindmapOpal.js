...
const MindMapPopup = ({ jsonData, onClose }) => {
  ...
  useEffect(() => {
    ...
    function update(source) {
      ...
      newNodes.forEach((d) => (d.y = d.depth * 280)); // Tăng khoảng cách để có chỗ cho thẻ chữ nhật lớn hơn
      ...
      function calculateNodeSize(d) {
        ...
        if (d.depth === 0) {
          // Root node: kích thước cố định
          return { width: 140, height: 50 };
        }

        // Tính chiều rộng dựa trên độ dài text
        const nameLength = name.length;
        const contentLength = content.length;

        // Chiều rộng tối thiểu và tối đa
        const minWidth = 120;
        const maxWidth = 250;

        // Tính chiều rộng dựa trên nội dung dài nhất
        ...
        // Tính chiều cao dựa trên số dòng content
        let height = 40; // Chiều cao cơ bản cho tên

        if (content) {
          // Ước tính số dòng cho content (khoảng 30-35 ký tự mỗi dòng với width hiện tại)
          const charsPerLine = Math.floor((width - 20) / 6); // 6px per char estimate
          const contentLines = Math.ceil(contentLength / charsPerLine);
          height += contentLines * 16 + 8; // 16px per line + margin
        }
